    ...
    render() {
        ...
        const arr = (litigants||[]).sort((a,b)=>a.createTime-b.createTime).map(function (i,idx) {
            ...
            return (<div className={style} key={idx}>
                <div className="margin-form-party"><span className="news-width font-big">当事人姓名：</span><span  className="show-style font-big" style={{ width: 90 }}>{i.name}</span></div>
                <div className="margin-form-party"><span className="news-width font-big" style={{width:42}}>性别：</span><span  className="show-style font-big" style={{ width: 65 }}>{i.sex === 1 ? '男':'女'}</span></div>
                ...
                <div className="margin-form-party"><span className="news-width font-big">单位或住址：</span><span className="address-width font-big" style={{ width: 436 }}>{i.address}</span></div>
