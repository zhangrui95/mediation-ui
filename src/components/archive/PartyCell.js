    ...
    render() {
        ...
        const arr = (litigants||[]).map(function (i,idx) {
            return (<div className="formArch" key={idx}>
                ...
                <div className="margin-form-party"><span className="news-width" style={{width:40}}>年龄：</span><span  className="show-style" style={{ width: 40 }}>{i.age}</span></div>
                <div className="margin-form-party"><span className="news-width">身份证号码：</span><span  className="show-style">{i.card}</span></div>
                <div className="margin-form-party"><span className="news-width">单位/住址：</span><span className="address-width" style={{ width: 460 }}>{i.address}</span></div>
